...
export function BookingForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const tr = selectedTour?.translations[language] || selectedTour?.translations['en'];
    const tourTitle = selectedTour ? (tr?.title || selectedTour.id) : formData.tour;
  ...
  const labels = {
    plan: { ka: 'დაგეგმე თავგადასავალი', en: 'Plan Your Adventure', ru: 'Запланируйте приключение', he: 'תכנן את ההרפתקה שלך' },
    desc: { ka: 'შეავსეთ ფორმა და ჩვენ 15 წუთში დაგიკავშირდებით.', en: 'Fill the form and we will contact you within 15 minutes.', ru: 'Заполните форму, и мы свяжемся с вами в течение 15 минут.', he: 'מלאו את הטופס ונחזור אליכם תוך 15 דקות.' },
    flexible: { ka: 'მოქნილი თარიღები', en: 'Flexible Dates', ru: 'Гибкие даты', he: 'תאריכים גמישים' },
    reschedule: { ka: 'უფასო გადაცვლა', en: 'Reschedule for free', ru: 'Перенос бесплатно', he: 'שינוי מועד בחינם' },
    discounts: { ka: 'ფასდაკლებები', en: 'Group Discounts', ru: 'Групповые скидки', he: 'הנחות לקבוצות' },
    secure: { ka: 'უსაფრთხო დაჯავშნა. წინასწარი გადახდის გარეშე.', en: 'Secure booking. No prepayment required.', ru: 'Безопасное бронирование. Без предоплаты.', he: 'הזמנה מאובטחת. אין צורך בתשלום מראש.' }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
              <div className="space-y-3">
                ...
                >
                  ...
                  {tours.map(tour => {
                    const tr = tour.translations[language] || tour.translations['en'] || tour.translations['ka'];
                    return (
                      <option key={tour.id} value={tour.id} className="text-gray-900 dark:text-white">{tr?.title || tour.id}</option>
                    );
                  })}
                </select>
              </div>
